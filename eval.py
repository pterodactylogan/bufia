...
'''
tier, fold number, licit/illicit/cur/att, test/dev
'''
def run_eval(t, fold, name, split, gpath):
    grammar = open("./data/quechua/Wilson_Gallagher/CrossValidationFolds/" + fold +
                   "/" + gpath + "/" + t + "_grammar" + fold + ".txt")
    features = open("./data/quechua/Wilson_Gallagher/features_wb.csv")

    test = open(("./data/quechua/Wilson_Gallagher/CrossValidationFolds/{0}/our_version/"+
...
for tier in ["succ", "laryngeal", "dorsal", "c-dorsal"]:
    print(tier)
        ...
        for name in ["cur", "att"]:
            for split in ["dev", "test"]:
                for sim in range(1,4):
                    run_eval(tier, str(fold), name,
                             split, "maxent/sim"+str(sim))
